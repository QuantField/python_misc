...
__CAT_LEVELS_MAX__ = 50 # allowed maximum levels for categorical data
...
def transToCategorical(data, nLevels=__CAT_LEVELS_MAX__):
...
def getImputeValue(data, missing=False, mean=False, threshold=0.8):
    #if missing is True a new level _MISS_ is created
    miss = countMissings(data) 
    missVals = {}
    toDrop   = []
    # fill missings if <= threshold and drop if > threshold
    # default threshold is 80% of missings
    for var in miss.index:
        if miss.ix[var]['Perc']!=0 and miss.ix[var]['Perc']<=threshold:
            typ = data[var].dtype
            # Numeric Data either with mode or mean
            if (typ == np.float64 or typ == np.int64): # numeric
                if mean==False:
                ...
                # if var is an identifer or unary then drop
                if levels>__CAT_LEVELS_MAX__ or levels==1: 
                    toDrop.append(var)  
                else:
                    if missing==True:
                        missVals.update({var:'_MISS_'})
                    else:
                        missVals.update({var:mode.at[0]})
        if miss.ix[var]['Perc']>threshold:
            toDrop.append(var)            
    return (missVals,toDrop)
  
def impute(data, missing=False, mean=False, imputeVal=None, 
           threshold=0.8, drop=False, toDrop=None): 
...
# information about numerical variables ( mean, min,...)    
def numVarInfo(data):
    s = {}
...
def toNumericB(data_in, keep=None, origVars = None):
    # keep is a list of normal + dummied variables
    MAX_LEVELS = 50
    data = data_in.copy()
    if keep==None:
        for r in data:
           ...
           if  not (dtyp in [np.float64, np.int64]) and len(data[r].unique())>MAX_LEVELS:   #levels max = 30
               print("dropped: "+ r)
               data.drop(r,1, inplace=True)
        tmp = pd.get_dummies(data, prefix_sep='$', drop_first=True)        
    else:
        tmp = pd.get_dummies(data[origVars], prefix_sep='$')
        tmp = tmp[keep]
    return tmp 
...
class processSteps:  # keeps track of transoformations       
    def __init__(self, folder = None):
        ...
        self.nLevels = __CAT_LEVELS_MAX__
    ...
    def copyTransfToCateg(self, obj=False, nLevels = __CAT_LEVELS_MAX__ ):
        self.transfToCateg = copy.deepcopy(obj) 
        self.nLevels  = copy.deepcopy(nLevels)
    ...
    def save(self):           
        if len(os.listdir(self.folder))<5: # simple way to check is already saved
            ...
            self.__dump(self.nLevels,'nLevels')
        else:
    ...
    def load(self):
        ...
        self.nLevels        = self.__load('nLevels')
